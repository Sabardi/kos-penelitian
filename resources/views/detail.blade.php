        ...
        <div class="max-w-screen-xl px-4 mx-auto 2xl:px-0">
            <div class="lg:grid lg:grid-cols-2 lg:gap-8 xl:gap-16">
                <div class="max-w-md mx-auto shrink-0 lg:max-w-lg">
                    <img class="w-full rounded-lg shadow-lg dark:hidden" src="{{ Storage::url($room->foto_room) }}" alt="{{ $room->name }}" />
                    <img class="hidden w-full rounded-lg shadow-lg dark:block" src="{{ Storage::url($room->foto_room) }}" alt="{{ $room->name }}" />
                </div>
                    ...
                    <div class="mt-4 space-y-4">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                            <p class="text-3xl font-extrabold text-gray-900 dark:text-white">
                                ...
                                <span class="text-lg font-medium text-gray-500">/ {{$room->property->time_period}}</span>
                            </p>
                            ...
                            <div class="flex items-center gap-2 mt-2 sm:mt-0">
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-2 text-sm font-medium text-gray-500 dark:text-gray-400">
                                        {{ number_format($room->average_rating, 1) }} ({{ $room->reviews->count() }} reviews)
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($room->facilities as $facility)
                                <span class="px-3 py-1 text-sm font-medium text-blue-800 bg-blue-100 rounded-full dark:bg-blue-900 dark:text-blue-300">
                                    {{ $facility->name }}
                                </span>
                            @endforeach
                        </div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($room->property->locations as $location)
                                <span class="px-3 py-1 text-sm font-medium text-gray-700 bg-gray-200 rounded-full dark:bg-gray-700 dark:text-gray-300">
                                    {{ $location->name }}
                                </span>
                            @endforeach
                        </div>
                                        ...
                                        <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                                            <div class="sm:flex sm:items-start">
                                                <div class="w-full mt-3 text-center sm:mt-0 sm:text-left">
                                                    ...
                                                    <form action="{{ route('front.booking', $room) }}" method="post" class="mt-6">
                                                        @csrf
                                                        <div class="space-y-4">
                                                            <div>
                                                                <label for="name" class="block text-sm font-medium text-gray-700">Nama Penyewa</label>
                                                                <input type="text" name="name" id="name" required class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                            </div>

                                                            <div>
                                                                <label for="number" class="block text-sm font-medium text-gray-700">Nomor Telepon</label>
                                                                <input type="text" name="number" id="number" required class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                            </div>

                                                            <div>
                                                                <label for="check_in" class="block text-sm font-medium text-gray-700">Tanggal Check-in</label>
                                                                <input type="date" name="check_in" id="check_in" required class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                            </div>
                                                        </div>
            ...
            <div class="max-w-screen-xl px-4 mx-auto">
                <div class="max-w-5xl mx-auto">
                    <div class="flex items-center justify-between mb-8">
                        ...
                        <select id="order-type" class="px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="mt-4" x-data="{ expanded: false }">
                                            <p class="text-gray-600 dark:text-gray-300">
                                                ...
                                                <span x-show="!expanded">{{ Str::words($review->comment, 10) }}...</span>
                                            </p>
